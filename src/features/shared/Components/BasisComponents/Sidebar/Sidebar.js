...
const Sidebar = ({colorSideBar}) => {
  ...
  return (
    <div className={`${colorSideBar === "light" ? 'bg-sidebar-bg' : 'bg-header-bg'} w-full h-full flex flex-col gap-4 transition-colors duration-500 ease`}>
        <div className='flex h-[60px] justify-center items-center'>
            ...
            <p className={`${state ? "text-2xl font-normal ml-2" : "w-0"} ${colorSideBar === "light" ? "text-black" : "text-white"}  transition-all duration-900 ease`}>{state ? "AtrioHR" : ""}</p>
        </div>
        <div className="flex flex-col justify-center items-center gap-2">
            ...
            <div className={`${state ?'flex flex-col items-center' : 'hidden'}`}>
                <p className={`${colorSideBar === "light" ? "text-black" : "text-white"} text-base font-semibold transition-all duration-900 ease`}>Emily Smith</p>
                <p className='text-[10px] text-[#919aa3]'>Manager</p>
            </div>
        </div>
        <div>
            <NavigationBar colorSideBar={colorSideBar}/>
        </div>
    </div>
  )
}
